...
def batch_predict_varying_T(latent_dim, covar_module0, covar_module1, likelihoods, prediction_x, 
    ...
    K1Xx_iH_K0xz = torch.zeros(latent_dim, test_x.shape[0], M, dtype=torch.double).to(device)
    K1XX = torch.zeros(latent_dim, test_x.shape[0], test_x.shape[0], dtype=torch.double).to(device)
    K1Xx_iH_K1xX = torch.zeros(latent_dim, test_x.shape[0], test_x.shape[0], dtype=torch.double).to(device)
    ...
    for s in test_subjects:
        ...
        if isinstance(covar_module0, list):
            ...
            K1XX[:, indices][:, :, indices] = covar_module1(stacked_test_x_indices, stacked_test_x_indices).evaluate()

            T = prediction_x[mask].shape[0]
            I_T = torch.eye(T, dtype=torch.double).to(device)

            B_st = covar_module1(stacked_prediction_x_mask,stacked_prediction_x_mask).evaluate() + I_T * likelihoods.noise_covar.noise.unsqueeze(dim=2)
            LB_st = torch.linalg.cholesky(B_st)
            iB_st = torch.cholesky_solve(I_T, LB_st)
            K0xz_st = K0xz[:, mask]

        K1Xx_mu_tilde[:, indices] = torch.matmul(K1Xx, mu_tilde[:, mask])
        K1Xx_iH_K0xz[:,indices,:] = torch.matmul(torch.matmul(K1Xx, iB_st), K0xz_st)
        K1Xx_iH_K1xX[:, indices][:, :, indices] = torch.matmul(torch.matmul(K1Xx, iB_st), K1Xx.transpose(-1, -2))

    Z_pred = (K0Xz_iK0zz_K0zx_mu_tilde + K1Xx_mu_tilde).squeeze(dim=2).T
    iK0zz_K0Xz_K1Xx_iH_K0xz_T = torch.linalg.solve(K0zz, (K0Xz - K1Xx_iH_K0xz).transpose(-1, -2))
    var_first_term = torch.matmul(torch.matmul((iK0zz_K0Xz_K1Xx_iH_K0xz_T).transpose(-1,-2), H), iK0zz_K0Xz_K1Xx_iH_K0xz_T)
    var_second_term = torch.eye(test_x.shape[0], dtype=torch.double).to(device) * likelihoods.noise_covar.noise.unsqueeze(dim=2) + K1XX - K1Xx_iH_K1xX
    H_var = var_first_term + var_second_term

    return Z_pred, H_var.T
