...
def validate(nnet_model, tpp, type_nnet, dataset, type_KL, num_samples, latent_dim_zy, latent_dim_zm, covar_module0_y,
             covar_module1_y, covar_module0_m, covar_module1_m, covar_module0_lambda, likelihoods_y, likelihoods_m,
             zt_list_y, zt_list_m, zt_list_lambda, m_lambda, L_lambda, tril_indices, beta, T, weight, id_covariate, disease_covariate, loss_function, m_y, m_m, H_y, H_m, P, natural_gradient=True, eps=1e-6):
    """
    Obtain loss of validation set.

    :param nnet_model: neural network model
    :param tpp: temporal point process model
    ...
    :param covar_module0_lambda: additive kernel of lambda
    ...
    :param zt_list_lambda: list of inducing points of lambda
    ...
    :param disease_covariate: covariate of presence of disease
    ...
    full_mu_y = torch.zeros(len(dataset), latent_dim_zy, dtype=torch.double, requires_grad=True).to(device)
    full_log_var_y = torch.zeros(len(dataset), latent_dim_zy, dtype=torch.double, requires_grad=True).to(device)
    full_mu_m = torch.zeros(len(dataset), latent_dim_zm, dtype=torch.double, requires_grad=True).to(device)
    full_log_var_m = torch.zeros(len(dataset), latent_dim_zm, dtype=torch.double, requires_grad=True).to(device)
    full_labels = torch.zeros(len(dataset), Q, dtype=torch.double, requires_grad=False).to(device)
    full_prev_timestamps = torch.zeros(len(dataset), tpp.D * 2, dtype=torch.double, requires_grad=False).to(device)
    full_labels_with_intensity = torch.zeros(len(dataset), Q + 1, dtype=torch.double, requires_grad=False).to(device)
    ...
    for batch_idx, sample_batched in enumerate(dataloader):
        ...
        sample_intensity = tpp.sample_intensity(m_lambda, L_lambda, tril_indices, beta_disease, zt_list_lambda,
                                                covar_module0_lambda, prev_timestamps)

        val_x_with_intensity = torch.cat((val_x, sample_intensity), dim=1)
        ...
        # mu_y = mu_y.detach()
        # full_mu_y[indices] = mu_y
        # full_log_var_y[indices] = log_var_y
        # full_mu_m[indices] = mu_m
        # full_log_var_m[indices] = log_var_m
        ...
        kld_loss_y, grad_m_y, grad_H_y = minibatch_KLD_upper_bound_iter(covar_module0_y, covar_module1_y, likelihoods_y, latent_dim_zy, m_y, PSD_H_y,
                                       val_x_with_intensity, mu_y, log_var_y, zt_list_with_intensity, P,  P_in_current_batch, N, natural_gradient, id_covariate, eps)

        kld_loss_m, grad_m_m, grad_H_m = minibatch_KLD_upper_bound_iter(covar_module0_m, covar_module1_m, likelihoods_m, latent_dim_zm, m_m, PSD_H_m,
                                       val_x_with_intensity, mu_m, log_var_m, zt_list_with_intensity, P,  P_in_current_batch, N, natural_gradient, id_covariate, eps)
        ...
        nll_loss_tpp, kld_loss_tpp = tpp.loss_tpp(prev_timestamps, t_n, subject_ids, zt_list_lambda,
                                                  covar_module0_lambda, m_lambda, L_lambda, tril_indices, beta_disease)

        nll_loss_tpp_sum += nll_loss_tpp
    ...
    # Do logging
    print('Validation set - Loss: %.3f  - GP loss: %.3f  - NLL loss: %.3f  - Recon Loss: %.3f - NLL Loss TPP: %.3f - GP Loss TPP: %.3f' % (
        net_loss_sum, gp_loss_sum, nll_loss_sum, recon_loss_sum, nll_loss_tpp_sum, kld_loss_tpp))
