...
def generate_kernel_batched(latent_dim, cat_kernel, bin_kernel, sqexp_kernel, cat_int_kernel, bin_int_kernel, covariate_missing_val,
                            id_covariate, scale=1, lengthscale=2.5):
    """
    ...
    :param covariate_missing_val:  list of dictionaries with indices from the covariate matrix with missing values
                            ...
                            E.g.: [{'covariate':0, 'mask': 3}]
    :param id_covariate:  index from the covariate matrix for the id covariate
    :param scale: scaling factor for marginal variance
    :param lengthscale: lengthscale of RBF kernels
    :return: AdditiveKernel without id covariate and AdditiveKernel with id covariate
    """
        ...
        if idx in covariate_missing:
            ...
            if idx == id_covariate:
                additive_kernel1.kernels.append(ScaleKernel(
                    CatKernel(active_dims=idx) *
                    BinKernel(active_dims=dict_instance['mask'], value=1),
                    batch_shape=torch.Size([latent_dim])).initialize(outputscale=torch.randn((latent_dim,1))**2 / scale))
            else:
                additive_kernel0.kernels.append(ScaleKernel(
                    CatKernel(active_dims=idx) *
                    BinKernel(active_dims=dict_instance['mask'], value=1),
            ...
            if idx == id_covariate:
                additive_kernel1.kernels.append(ScaleKernel(CatKernel(active_dims=idx), batch_shape=torch.Size([latent_dim])).initialize(outputscale=torch.randn((latent_dim,1))**2 / scale))
            else:
                additive_kernel0.kernels.append(ScaleKernel(CatKernel(active_dims=idx), batch_shape=torch.Size([latent_dim])).initialize(outputscale=torch.randn((latent_dim,1))**2 / scale))
    ...
    for idx in sqexp_kernel:
        if idx in covariate_missing:
            dict_instance = covariate_missing_val[covariate_missing.index(idx)]
            additive_kernel0.kernels.append(ScaleKernel(
                RbfKernel(active_dims=idx, batch_shape=torch.Size([latent_dim]), lengthscale=lengthscale) *
                BinKernel(active_dims=dict_instance['mask'], value=1),
                batch_shape=torch.Size([latent_dim])).initialize(outputscale=torch.randn((latent_dim,1))**2 / scale))
        else:
            additive_kernel0.kernels.append(ScaleKernel(RbfKernel(active_dims=idx, batch_shape=torch.Size([latent_dim]), lengthscale=lengthscale),
                                                        batch_shape=torch.Size([latent_dim])).initialize(outputscale=torch.randn((latent_dim,1))**2 / scale))
    ...
    for dict_instance_kernel in cat_int_kernel:
        ...
        if dict_instance_kernel['cont_covariate'] in covariate_missing:
            dict_instance = covariate_missing_val[covariate_missing.index(dict_instance_kernel['cont_covariate'])]
            masked_kernel2 = RbfKernel(active_dims=dict_instance_kernel['cont_covariate'], batch_shape=torch.Size([latent_dim]), lengthscale=lengthscale) * BinKernel(
                active_dims=dict_instance['mask'], value=1)
        else:
            masked_kernel2 = RbfKernel(active_dims=dict_instance_kernel['cont_covariate'], batch_shape=torch.Size([latent_dim]), lengthscale=lengthscale)
