...
def save_data_irregular(data_file, mask_file, data_masked_file, label_file, rotated_MNIST, label_dict, mask, columns_of_prev_times):
...
def generate_mask_mnar_irregular(image, sick_var, rotation, shape): # for sick instances box increases in time, moving along the diagonal
    image = image / 255
    p = (image / 2) * 1.5
    ...
    for digit in digit_mod.keys():
        ...
        for i in range(digit_mod[digit]):
            ...
            for idx, rotation in enumerate(rotations):
                ...
                number_of_timestamps = int(min(idx + 1, D))

                prev_time_stamps = (time_age[idx] - np.flip(time_with_zero[:idx + 1]))[
                                   :number_of_timestamps].tolist() + (D - number_of_timestamps) * [np.nan]
                mask_of_prev_time = [1] * number_of_timestamps + [0] * (D - number_of_timestamps)
                ...
                if sick_var == 1:
                    label_dict[sample_index] = \
                        [subject_index, digit, rotation, sick_var, gender, time_age[idx],
                         idx] + prev_time_stamps + mask_of_prev_time
                elif sick_var == 0:
                    label_dict[sample_index] = [subject_index, digit, rotation, sick_var, gender,
                                                time_age[idx], idx] + prev_time_stamps + mask_of_prev_time
            ...
            if i % 200 == 199:
                ...
                save_data_irregular(data_file, mask_file, data_masked_file, label_file,
                          rotated_MNIST, label_dict, mask, columns_of_prev_times)
                rotated_MNIST = np.empty((0, 1296))
                mask = np.empty((0, 1296))
                label_dict = {}
