...
def hensman_training(nnet_model, tpp, type_nnet, epochs, dataset, optimiser, type_KL, num_samples, latent_dim_zy,latent_dim_zm, covar_module0_y,
                     covar_module1_y, covar_module0_m,covar_module1_m, covar_module0_lambda, likelihoods_y, likelihoods_m, m_y, H_y, m_m, H_m, m_lambda, L_lambda, tril_indices,beta,
                     zt_list_y, zt_list_m, zt_list_lambda, P, T, varying_T, Q, weight, id_covariate, disease_covariate, loss_function,
                     ...
                     prediction_dataset=None, gp_model_y=None, gp_model_m=None, gp_model_lambda=None,  csv_file_validation_data=None, csv_file_validation_label=None,
    ...
    :param tpp: temporal point process class
    ...
    :param covar_module0_lambda: additive kernel (sum of cross-covariances) for temporal point process
    ...
    :param m_lambda: variational mean for z_lambda
    :param L_lambda: variational cholesky factor for z_lambda
    ...
    :param zt_list_lambda: list of inducing points for z_lambda
    ...
    :param disease_covariate: covarite of present of disease
    ...
    :param gp_model_lambda: GPyTorch gp model for z_lambda
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch_idx, sample_batched in enumerate(dataloader):
            ...
            # sample intensity value that is used as input for kernel zy and zm
            sample_intensity = tpp.sample_intensity(m_lambda, L_lambda, tril_indices, beta_disease, zt_list_lambda, covar_module0_lambda, prev_timestamps)
            train_x_with_intensity = torch.cat((train_x, sample_intensity), dim=1)
            ...
            sample_intensity_z = tpp.sample_intensity(m_lambda, L_lambda, tril_indices, beta_disease_zt_list, zt_list_lambda, covar_module0_lambda, zt_list_lambda)
            sample_intensity_z = torch.transpose(sample_intensity_z,0,1).expand(zt_list_y.shape[0], zt_list_y.shape[1])
            zt_list_with_intensity = torch.cat((zt_list_y, torch.unsqueeze(sample_intensity_z,2)), dim=2)

            # for i in range(zt_list_y.shape[0]):
            #     sample_intensity_z = tpp.sample_intensity(m_lambda, L_lambda, tril_indices, beta, zt_list_lambda, covar_module0_lambda, zt_list_lambda)
            #     zt_list_with_intensity[i] = torch.cat((zt_list_y[i], sample_intensity_z), dim=1)
            ...
            if varying_T:
                ...
                kld_loss_y, grad_m_y, grad_H_y = minibatch_KLD_upper_bound_iter(covar_module0_y, covar_module1_y, likelihoods_y, latent_dim_zy, m_y, PSD_H_y, train_x_with_intensity, mu_y, log_var_y, zt_list_with_intensity, P, P_in_current_batch, N, natural_gradient, id_covariate, eps)
                kld_loss_m, grad_m_m, grad_H_m = minibatch_KLD_upper_bound_iter(covar_module0_m, covar_module1_m, likelihoods_m, latent_dim_zm, m_m, PSD_H_m, train_x_with_intensity, mu_m, log_var_m, zt_list_with_intensity, P, P_in_current_batch, N, natural_gradient, id_covariate, eps)
            else:
                P_in_current_batch = N_batch // T
                kld_loss_y, grad_m_y, grad_H_y = minibatch_KLD_upper_bound(covar_module0_y, covar_module1_y, likelihoods_y, latent_dim_zy, m_y, PSD_H_y, train_x_with_intensity, mu_y, log_var_y, zt_list_with_intensity, P, P_in_current_batch, T, natural_gradient, eps)
                kld_loss_m, grad_m_m, grad_H_m = minibatch_KLD_upper_bound(covar_module0_m, covar_module1_m, likelihoods_m, latent_dim_zm, m_m, PSD_H_m,train_x_with_intensity, mu_m, log_var_m, zt_list_with_intensity, P,P_in_current_batch, T, natural_gradient, eps)
        ...
        print('Iter %d/%d - Loss: %.3f  - GP loss y: %.3f  - NLL Loss y: %.3f  - Recon Loss y: %.3f - GP_mask loss: %.3f  - NLL_mask Loss: %.3f  - Recon Loss_mask: %.3f' % (
                epoch, epochs, net_loss_sum, kld_loss_sum_y, nll_loss_sum_y, recon_loss_sum_y, kld_loss_sum_m,
                nll_loss_sum_m, recon_loss_sum_m), flush=True)
        ...
        if (not epoch % 25) and epoch != epochs:
            with torch.no_grad():
                ...
                if validation_dataset is not None:
                    ...
                    val_pred_mse = validate(nnet_model, tpp, type_nnet, validation_dataset, type_KL, num_samples, latent_dim_zy, latent_dim_zm,covar_module0_y, covar_module1_y,
                                    covar_module0_m, covar_module1_m, covar_module0_lambda, likelihoods_y,likelihoods_m, zt_list_y, zt_list_m, zt_list_lambda,
                                            m_lambda, L_lambda, tril_indices, beta, T, weight, id_covariate, disease_covariate, loss_function, m_y, m_m, H_y, H_m, P, natural_gradient, eps=1e-6)
                    ...
                    if val_pred_mse < best_val_pred_mse:
                        ...
                        try:
                            ...
                            torch.save(gp_model_lambda.state_dict(), os.path.join(results_path, 'gp_model_lambda_best.pth'))
                            torch.save(zt_list_lambda, os.path.join(results_path, 'zt_list_lambda_best.pth'))
                            torch.save(m_lambda, os.path.join(results_path, 'm_lambda_best.pth'))
                            torch.save(L_lambda, os.path.join(results_path, 'L_lambda_best.pth'))
                            torch.save(beta, os.path.join(results_path, 'beta_best.pth'))
